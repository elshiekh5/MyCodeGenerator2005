			...
			foreach (SQLDMO.Column column in Fields)
            {
                ...
                if (ID == null || column.Name != ID.Name)//||!Globals.CheckIsAddedBySql(ID))
                {
					...
					if (Globals.GetSqlDataType(column.Datatype) == SqlDbType.DateTime)
					{
						...
						controls.Append("\n\t\t\t\t\t\t<input id=\"txt" + Globals.GetProgramatlyName(column.Name) + "\" runat=\"server\" class=\"Control\" style=\"width:250px\" type=\"text\" readonly=\"readonly\" /><a href=\"javascript:NewCal('ctl00$BasicContent$txt" + Globals.GetProgramatlyName(column.Name) + "\','ddmmyyyy')\"><img src=\"/images/cal.gif\" width=\"16\" height=\"16\" border=\"0\" alt=\"«Œ — «· «—ÌŒ\"></a>");
